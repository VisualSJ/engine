...
var UuidUtils = {
    ...
    compressUuid: function(uuid, min) {
        ...
        } else if (Reg_subAssetUuid.test(uuid)) {
            let reserved = (min === true) ? 2 : 5;
            let result = uuid.match(Reg_subAssetUuid);
            if (result) {
                uuid = result[1];
                return UuidUtils.compressHex(uuid, reserved) + result[2];
            }
