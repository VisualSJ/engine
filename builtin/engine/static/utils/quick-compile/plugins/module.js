...
const QUICK_COMPILE_FILE = Path.join(__dirname, QUICK_COMPILE_FILE_NAME);

const QUICK_COMPILE_CONTENT = Fs.readFileSync(QUICK_COMPILE_FILE, 'utf8');
...
module.exports = function(opts) {
    ...
    return {
        ...
        transform(script, compiler) {
            ...
            let oldMap = oldMapObj && oldMapObj.toObject();
            let newMap = JSON.parse(gen.map.toString());
            let mergedMap = merge(oldMap, newMap);
            let mapComment = convert.fromObject(mergedMap).toComment();
