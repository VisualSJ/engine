...
class Panel {
     ...
     */
    close(name) {
        ipc.send(`editor-lib-panel:call`, 'close', name);
    }
     ...
     */
    edit(action) {
        ipc.send(`editor-lib-panel:call`, 'edit', action);
    }
}
 ...
 * 主进程关闭某个面板
 */
ipc.on(`editor-lib-panel:close`, async (event, name) => {
    let $panel = panel.query(name);
    if (!$panel) {
        return event.reply(null, true);
    }
    const result = await $panel.parentElement.$groups.removePanel(name);
