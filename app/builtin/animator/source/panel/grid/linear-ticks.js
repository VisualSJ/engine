...
class LinearTicks {
    ...
    initTicks(lods, min, max) {
        ...
        while (curTick * this.tickLods[curlodIdx] <= maxScale) {
        ...
        curTick = 1.0;
        while (curTick / this.tickLods[curlodIdx] >= minScale) {
            curTick = curTick / this.tickLods[curlodIdx];
    ...
    range(minValue, maxValue, pixelRange) {
        ...
        for (let j = this.minTickLevel; j <= this.maxTickLevel; j++) {
            this.tickRatios[j] = EditorMath.clamp01(this.tickRatios[j]);
        }
