...
class RopeJointGizmo extends JointGizmo {
    createAnchorGroup () {
        let root = this._root;
    ...
    createToolGroup () {
        ...
        let fgLine = group.line()
            .stroke({width:3, color: '#00cccc', opacity: 0.5})
            ;
        ...
        group.plot = (args) => {
            if (args.distance) {
                ...
                fgLine.style('stroke-dasharray', Tools.dashLength([5, 3]));
    ...
    createArgs () {
        ...
        if (this.target.connectedBody) {
            ...
            let d = cc.v2(this.target.maxLength, 0);
