 ...
 */
const onDomUtilsDrag = function(elem, detach = false) {
    ...
    if (!detach) {
        ...
        elem._dragHandles = (function (elem) {
            ...
            const _onHueCtrlDragStart = event => {
                elem.dragging = true;
                elem.oldValue = elem.value;
            };

            const _onHueCtrlDragging = event => {
            ...
            const _onColorCtrlDragStart = event => {
                elem.dragging = true;
                elem.oldValue = elem.value;
            };

            const _onColorCtrlDragging = event => {
            ...
            const _onColorCtrlDragEnd = event => {
                if (elem.dragging) {
                    elem.dragging = false;
                    const e = 360 * (1 - parseFloat(elem.$hueHandle.style.top) / 100);
                    const i = elem.value[3];
            ...
            const _onAlphaCtrlDragStart = event => {
                elem.dragging = true;
                elem.oldValue = elem.value;
            };

            const _onAlphaCtrlDragging = event => {
            ...
            const _onAlphaCtrlDragEnd = event => {
                if (elem.dragging) {
                    elem.dragging = false;
                    const rect = elem.$alphaCtrl.getBoundingClientRect();
                    const top = clamp((event.clientY - rect.top) / elem.$alphaCtrl.clientHeight, 0, 1);
            ...
            return {
                ...
                _onHueCtrlDragging,
                _onHueCtrlDragEnd,
                _onColorCtrlDragStart,
                _onColorCtrlDragging,
                _onColorCtrlDragEnd,
                _onAlphaCtrlDragStart,
                _onAlphaCtrlDragging,
                _onAlphaCtrlDragEnd
            }
        })(elem);
    }
    ...
    DomUtils.controlDragEvent(
        elem.$hueCtrl,
        {
            start: elem._dragHandles._onHueCtrlDragStart,
            drag: elem._dragHandles._onHueCtrlDragging,
            end: elem._dragHandles._onHueCtrlDragEnd
        },
        detach
    );
    ...
    DomUtils.controlDragEvent(
        elem.$colorCtrl,
        {
            start: elem._dragHandles._onColorCtrlDragStart,
            drag: elem._dragHandles._onColorCtrlDragging,
            end: elem._dragHandles._onColorCtrlDragEnd
        },
        detach
    );
    ...
    DomUtils.controlDragEvent(
        elem.$alphaCtrl,
        {
            start: elem._dragHandles._onAlphaCtrlDragStart,
            drag: elem._dragHandles._onAlphaCtrlDragging,
            end: elem._dragHandles._onAlphaCtrlDragEnd
        },
        detach
    );
...
class ColorPicker extends Base {
    ...
    */
    ...
    constructor () {
        ...
        this.$hueHandle.$root = this.$colorHandle.$root = this.$alphaHandle.$root = this.$hueCtrl.$root = this.$colorCtrl.$root = this.$alphaCtrl.$root = this.$btnAdd.$root = this.$palette.$root = this;
        this.$sliderR.$root = this.$sliderG.$root = this.$sliderA.$root = this.$sliderB.$root = this.$newColor.$root = this.$oldColor.$root = this.$hexInput.$root = DomUtils.$root = this;
    ...
    disconnectedCallback () {
        ...
        // 存储color presets数据
        // localStorage.setItem('colorPresets', JSON.stringify(this._colorPresets));
