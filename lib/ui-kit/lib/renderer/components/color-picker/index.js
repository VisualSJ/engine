...
class ColorPicker extends Base {
    ...
    get value() {
        ...
        try {
            ...
            value = val ? Chroma(val).rgba() : [0, 0, 0, 1];
            value[3] = value[3] * 255;
        }
        return value;
    }
    ...
    _updateColorDiff() {
        try {
            this.lastAssignValue = JSON.parse(this.lastAssignValue);
            this.lastAssignValue[3] = this.lastAssignValue[3] / 255;
        } catch (error) {
        ...
        value[3] = value[3] / 255;
        this.$newColor.style.backgroundColor = Chroma(value).css();
    }

    _updateHue() {
        let temp = this.value;
        temp[3] = temp[3] / 255;
        const value = Chroma(temp).hsv();
        if (isNaN(value[0])) {
            value[0] = 360;
        }
    ...
    _updateAlpha() {
        ...
        value[3] = value[3] / 255;
        this.$alphaCtrl.style.background = `linear-gradient(${Chroma(value).hex()},rgba(0, 0, 0,0))`;
    ...
    _updateHexInput() {
        let value = Array.from(this.value);
        value[3] = value[3] / 255;
        this.$hexInput.value = Chroma(value)
            .hex()
            .toUpperCase();
    }
