...
// 默认颜色
const DEFAULT_CORLOR  = [0, 0, 0, 1];

/**
 * 添加和解绑监听事件
 * 调用外部的 drag 方法，实现拖到事件控制
 * 因为事件在 domUtils 内，而事件内需要调用当前对象只能放在外部通过此方法使用
 * @param {Element} elem
    ...
    if (!detach) {
        ...
        elem._dragHandles = (function(elem) {
            ...
            const onHueCtrlDragging = (event) => {
                ...
                const l = 360 * (1 - top);
                const a = Chroma(elem._getRgba()).hsv();
            ...
            const onHueCtrlDragEnd = (event) => {
                if (elem.dragging) {
                    ...
                    !elem.type && (elem.type = 'hueChange');
                    elem.value = value;
                    // 在黑白灰区域，色相调节不会更改 rgb 值，因而不能根据值是否改变来判断 change 和 confirm 的触发机制，有拖动都需要触发）
                    elem.dispatch('change');
                    elem.dispatch('confirm');
                    elem._staging =  Array.from(value);
                    elem._changeStage = Array.from(value);
                }
            };
...
class ColorPicker extends Base {
    ...
    connectedCallback() {
        ...
        this.lastAssignValue = this._getRgba(this.oldValue);
        // 初始化获取 color presets 数据
    ...
    get value() {
        ...
        try {
            ...
            value = val ? Chroma(val).rgba() : Array.from(DEFAULT_CORLOR);
            value[3] = value[3] * 255;
        }
        return value;
    }
    ...
    /**
     * 控制 colorPicke r的显示
     * 每次显示需要做 colorPresets 的初始化工作，以保持该内容可以实时同步
     */
    show() {
        // 初始化获取 color presets 数据
        if (localStorage.getItem('colorPresets')) {
    ...
    _onKeyDown(event) {
        switch (event.keyCode) {
            case 13: // 回车
                // 冒泡 enterKey 特殊事件，便于父级处理
                this.dispatch('enterKey');
                this.dragging = false;
                this._confirm();
                this._staging = Array.from(this.value);
                break;
            case 27: // esc
                // 冒泡 escKey 特殊事件，便于父级处理
                this.dispatch('escKey');
    ...
    /**
     * 根据传入的颜色，生成颜色格子标签，并返回该 dom 元素
     * @param {String} color 需要是能直接作为 css 的颜色值，使用该函数前需要做好数据处理
     */
    ...
    /**
     * 右键点击 colorPresets 内的颜色格子，右键弹出 palette 的颜色处理菜单
     * @param {Event} event
     */
     ...
     */
    _getRgba(val = this.value || DEFAULT_CORLOR) {
        // 值无效取默认值
        !val && (val = Array.from(DEFAULT_CORLOR));
        // 值非数组（字符串）用 chroma 转换
        !Array.isArray(val) && (val = Chroma(val).rgba());
    ...
    _updateColorDiff() {
        ...
        this.$oldColor.style.backgroundColor = this.lastAssignValue;
        this.$newColor.style.backgroundColor = this._getRgba();
    }

    _updateHue() {
        const value = Chroma(this._getRgba()).hsv();
        if (isNaN(value[0])) {
            value[0] = 360;
        }
    ...
    // 更新 Alpha 值
    _updateAlpha() {
        this.$alphaCtrl.style.background =
        `linear-gradient(${Chroma(this._getRgba()).hex()},rgba(0, 0, 0,0))`;
        this.$alphaHandle.style.top = `${100 * (1 - value[3])}%`;
    }
    ...
    _updateSliders() {
        ...
        // ts@ignore
        this.$sliderA.value = this.value[3];
    }
     ...
     */
    _updateHexInput() {
        this.$hexInput.value = Chroma(this._getRgba())
            .hex()
            .toUpperCase();
    }
