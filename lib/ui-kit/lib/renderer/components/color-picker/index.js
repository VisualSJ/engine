    ...
    if (!detach) {
        ...
        elem._dragHandles = (function(elem) {
            ...
            const onHueCtrlDragging = (event) => {
                ...
                const l = 360 * (1 - top);
                const a = Chroma(domUtils.changeToRgba(elem.value)).hsv();
...
class ColorPicker extends Base {
    ...
    connectedCallback() {
        ...
        this.lastAssignValue = domUtils.changeToRgba(this.oldValue);
    ...
    _onPaletteClick(event) {
        let color = Chroma(event.target.style.backgroundColor).rgba();
        color[3] *= 255;
    ...
    _updateColorDiff() {
        ...
        this.$newColor.style.backgroundColor = domUtils.changeToRgba(this.value);
    }

    _updateHue() {
        const value = Chroma(domUtils.changeToRgba(this.value)).hsv();
        if (isNaN(value[0])) {
            value[0] = 360;
        }
    ...
    _updateAlpha() {
        ...
        `linear-gradient(${Chroma(domUtils.changeToRgba(this.value)).hex()},rgba(0, 0, 0,0))`;
        this.$alphaHandle.style.top = `${100 * (1 - this.value[3] / 255)}%`;
    }
     ...
     */
    _updateHexInput() {
        this.$hexInput.value = Chroma(domUtils.changeToRgba(this.value))
            .hex()
            .toUpperCase();
    }
