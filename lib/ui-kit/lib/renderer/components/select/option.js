...
const controlMouseEvent = function (instance, detach = false) {
    const onClick = event => {
        if (!instance.disabled && !instance.grougDisabled) {
            DomUtils.acceptEvent(event);
            DomUtils.fire(instance, 'optionClick', {
                bubbles: true,
                detail: { value: instance.value ,valueText:instance.innerHTML}
            });
        }
    };
    const onMouseEnter = () => {
        instance.parentNode.hoverindex = [...instance.parentNode.children].indexOf(instance);
    };
    if (detach) {
        instance.removeEventListener('mouseenter', onMouseEnter, false);
        instance.removeEventListener('click', onClick, false);
    } else {
        instance.addEventListener('mouseenter', onMouseEnter, false);
