...
class Input extends Base {
     ...
     */
    _onInputFocus() {
        this._root._staging = this._root.value;
    }
     ...
     */
    _onInputBlur() {
        this._root.dispatch('confirm');
        delete this._root._staging;
    }
     ...
     */
    _onInputChange() {
        this._root.value = this.value;
        this._root.dispatch('change');
    }
    ...
    _onInputKeyDown(event) {
        switch (event.keyCode) {
            case 13: // 回车
                ...
                break;
            case 27: // esc
                this._root.value = this._root._staging;
                this._root.dispatch('change');
                this._root.dispatch('cancel');
                break;
        }
    }
}
