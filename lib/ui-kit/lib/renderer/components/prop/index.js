...
let defaultCreateUI = (type, data) => {
    ...
    switch (type) {
        ...
        case 'enums':
            if (data.optionValue) {
              // 如果传入的optionValue 与 oprions长度不同
...
class Prop extends Base {
    ...
    _updateUI() {
        ...
        if (this.type === 'enums') { // 生成select组件的数据传递方式不同需要单独处理
            ...
            html = createUI(this.type, {options, value, optionValue});
            // 若html返回为假值则调用默认生成UI方法
            if (!html) {
                html = defaultCreateUI(this.type, {options, value, optionValue});
            }
        } else {
