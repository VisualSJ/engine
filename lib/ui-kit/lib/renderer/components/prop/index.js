...
let createUI = (type, data) => {
    ...
    switch (type) {
        ...
        case 'enums':
            ...
            html = `<ui-select value="${value.value}">
                        ${Object.values(value.option).map(item => "<option value=" + item + ">" + item + "</option>").join('')}
                  </ui-select>`;
            break;
        case 'vec2':
            value = JSON.parse(data);
        ...
        case 'color4':
            value = JSON.parse(data);
            html = `<ui-color  value=${JSON.stringify(Object.values(value).map((item, index)=>index === 3 ? item : item * 255))}></ui-color>`;
            break;
        case 'quat':
            value = JSON.parse(data);
            html = `<ui-num-input preci="2"  value=${value.x} unit="X"></ui-num-input>
...
class Prop extends Base {
    ...
    _updateUI() {
        ...
        let html = createUI(this.type, this.value);
 ...
 */
const _mouseContron = (elem, detach = false) => {
