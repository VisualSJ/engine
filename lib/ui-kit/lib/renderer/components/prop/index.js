 ...
 * @param {String | Object} data  // 对象或字符串数据
 * @param {JSON Object} json 字符串数据
 */
let defaultCreateUI = (type, data, config) => {
    let html = '';
    let attrConfig = '';
    // 处理属性配置数据
    if (config) {
        try {
            ...
            Object.keys(config).forEach((key) => {
                // 为避免属性之间连在一起，需要流一个空格
                attrConfig += ` ${key}=${config[key]}`;
            });
        } catch (err) {
            console.warn('ui-prop:the config is invalid, please check it again');
        }
    }
    switch (type) {
        case 'string': // 普通的 text 节点
            html = `<ui-input value =${data}  ${attrConfig}></ui-input>`;
            break;
        case 'boolean':
            html = `<ui-checkbox value =${data}  ${attrConfig}></ui-checkbox>`;
            break;
        case 'int':
            html = `<ui-num-input value =${data} ${attrConfig}></ui-num-input>`;
            break;
        case 'number':
            html = `<ui-num-input value =${data} ${attrConfig}></ui-num-input>`;
            break;
        case 'enums':
            ...
            data.options.forEach((item, index) => {
                if (typeof item === 'object') {
                    ...
                    optionHtml += `<option value=${index}>${item}</option>`;
                }
            });
            html = `<ui-select value=${data.value}  ${attrConfig}>${optionHtml}</ui-select>`;
            break;
        case 'quat':
        case 'vec2':
            html = `<ui-prop type="number" label="X" value=${data.x} path="x"  ${attrConfig}></ui-prop>
                <ui-prop type="number" label="Y" value=${data.y} path="y"  ${attrConfig}></ui-prop>`;
            break;
        case 'size':
            html = `<ui-prop type="number" label="W" value=${data.width} path="width"  ${attrConfig}></ui-prop>
                <ui-prop type="number" label="H" value=${data.height} path="height"  ${attrConfig}></ui-prop>`;
            break;
        case 'vec3':
            html = `<ui-prop type="number" label="X" value=${data.x} path="x"  ${attrConfig}></ui-prop>
                    <ui-prop type="number" label="Y" value=${data.y} path="y"  ${attrConfig}></ui-prop>
                    <ui-prop type="number" label="Z" value=${data.z} path="z"  ${attrConfig}></ui-prop>`;
            break;
        case 'color':
            html = `<ui-color value=${JSON.stringify(data)}></ui-color>`;
            break;
        case 'color3':
        case 'color4':
            html = `<ui-color value=${JSON.stringify(data)}></ui-color>`;
...
class Prop extends Base {
    ...
    attributeChangedCallback(attr, oldData, newData) {
        switch (attr) {
            ...
            case 'options':
                ...
                break;
            case 'tabindex':
                this.setAttribute('tanindex', null);
                break;
            case 'value':
     ...
     */
    _updateUI() {
        if (!this.$slotCon || this.type == null) {
            return;
        }
        ...
        if (this.type === 'enums') { // 生成 select 组件的数据传递方式不同需要单独处理
            ...
            // 先获取正确值
            value = this._checkValue(this.getAttribute('value'));
            // 多个 ui-prop 组合的组件，为保证样式间距正常，需要加类名作样式特殊处理
            if (this.type === 'vec2' || this.type === 'vec3' || this.type === 'size') {
                this.className = 'vector';
            }
            html = createUI(this.type, value, this.config);
            // 若 html 返回为假值则调用默认生成UI方法
            if (!html) {
                html = defaultCreateUI(this.type, value, this.config);
            }
        }
    ...
    _updateValue() {
        ...
        let value = this._checkValue(this.getAttribute('value'));
        switch (this.type) {
            // 单类型，直接修改即可
            case 'enums':
            case 'number':
            ...
            case 'input':
            case 'string':
                this.updateUIAttr('value', value, this.$slotCon);
                break;
            // 复合类型
            default:
                for (let item of this.$slotCon.childNodes) {
                    if (!domUtils.isUIComponent(item)) {
                        continue;
                    }
                    item.setAttribute('value', value[item.path] || JSON.stringify(value));
                }
                break;
        }
    }
    ...
    _checkValue(value) {
        switch (this.type) {
            ...
            case 'color':
                ...
                value = Object.values(value).map((item, index) => index === 3 ? (item / 255) : +item);
                break;
            case 'color3':
            case 'color4':
                if (!value) {
                ...
                value = Object.values(value).map((item, index) => index === 3 ? +item : item * 255);
                break;
            default:
                break;
        }
    ...
    _mouseUp(event) {
        ...
        // label 点击触发的事件，把焦点转移到第一个ui子元素上
        this._setFocusToFirst(this.$slotCon);
    }

    // 转移焦点到第一个非 ui-prop 得 ui 元素上
    _setFocusToFirst($father) {
        let length = $father.childNodes.length;
        for (let i = 0; i < length; ++i) {
            if (domUtils.isUIComponent($father.childNodes[i])) {
                if ($father.childNodes[i].tagName === 'UI-PROP') {
                    this._setFocusToFirst($father.childNodes[i].$slotCon);
                    return;
                }
                $father.childNodes[i].focus();
                this._focusElement = $father.childNodes[i];
                this._hasFocus = true;
                return;
            }
        }
...
const mouseContron = (elem, detach = false) => {
    elem._domHandle = (function(elem) {
        ...
        const mouseUp = (event) => {
            elem._labelClick = false;
            if (!elem._focusElement) {
                return;
            }
            if (elem.$slotCon && elem._isDrag && !elem._hasFocus) {
                elem._focusElement.focus();
                elem._focusElement = null;
            }
            elem._isDrag = false;
        };
