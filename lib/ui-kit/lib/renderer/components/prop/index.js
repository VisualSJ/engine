...
let defaultCreateUI = (type, data, config) => {
    ...
    switch (type) {
        ...
        case 'vec2':
            ...
            html = `<ui-prop type="number" label="X" value=${value.x} path="x"  ${attrConfig}></ui-prop>
                <ui-prop type="number" label="Y" value=${value.y} path="y"  ${attrConfig}></ui-prop>`;
            break;
        case 'size':
            if (!data) {
                value = {
            ...
            html = `<ui-prop type="number" label="W" value=${value.width} path="width"  ${attrConfig}></ui-prop>
                <ui-prop type="number" label="H" value=${value.height} path="height"  ${attrConfig}></ui-prop>`;
            break;
        case 'vec3':
            if (!data) {
                value = {
            ...
            html = `<ui-prop type="number" label="X" value=${value.x} path="x"  ${attrConfig}></ui-prop>
                    <ui-prop type="number" label="Y" value=${value.y} path="y"  ${attrConfig}></ui-prop>
                    <ui-prop type="number" label="Z" value=${value.z} path="z"  ${attrConfig}></ui-prop>`;
            break;
        case 'color':
            if (!data) {
                value = [0, 0, 0, 255];
...
class Prop extends Base {
    ...
    _updateUI() {
        ...
        if (this.type === 'enums') { // 生成 select 组件的数据传递方式不同需要单独处理
            ...
            html = createUI(this.type, this.value, this.config);
            // 若 html 返回为假值则调用默认生成UI方法
            if (!html) {
                html = defaultCreateUI(this.type, this.value, this.config);
            }
        }
