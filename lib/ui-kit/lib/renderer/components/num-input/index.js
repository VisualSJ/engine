...
class NumInput extends Base {
    ...
    stepUp() {
        this.focused = true;
        let value = mathUtils.accAdd(this.$input.value, this.step);
        if (value > this.max) {
            if (this._timer) {
                // 清除定时器
    ...
    stepDown() {
        this.focused = true;
        let value = mathUtils.accSub(this.$input.value, this.step);
        if (value < this.min) {
            if (this._timer) {
                // 清除定时器
                clearTimeout(this._timer);
    ...
    /**
     * down 按键点击事件,调用 input 原生的减少数据接口
     */
