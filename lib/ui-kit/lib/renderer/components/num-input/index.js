...
class NumInput extends Base {
    ...
    disconnectedCallback() {
        ...
        this.$up.removeEventListener('mouseup', this._onMouseUp);
        this.$down.removeEventListener('mouseup', this._onMouseUp);
    }
    ...
    _onKeyDown(event) {
        switch (event.keyCode) {
            ...
            case 13: // enter
                // 值未发生变化
                if (this._staging === this.getAttribute('value')) {
                    // 当值与原值相等时,需要判断是否是由于只输入负号而导致的数据未更新
                    break;
                }
                // confirm之前需要更改暂存数据值
                this._staging = this.getAttribute('value');
    ...
    */
    ...
    /**
     * input的blur事件,当值发生改变时,离开input则视为确认该值
     * @param {Event} event 
     */
    _onInputBlur(event) {
        // 判断是否为可读或禁用
        ...
        // 当值未发生改变或者输入无效值，不触发confirm并恢复原值
        if (!this.$root.getAttribute('value') || this.$root._staging === this.$root.getAttribute('value') || this.$root.getAttribute('value') === '') {
            // 当前输入框内为负号时,confirm前需要将输入框内容更新为原值
            this.value = this.$root._staging;
            this.$root.setAttribute('value', this.$root._staging);
