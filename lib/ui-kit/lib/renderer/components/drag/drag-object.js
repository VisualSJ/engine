...
let focusedAction = function(uuid) {
    return uuid;
};
...
class DragObject extends Base {
    ...
    _onAreaClick() {
        // 获得焦点后需要闪烁下资源
        if (this.$root.hasAttribute('focused') && this.$root.value) {
            focusedAction(this.$root.value);
        }
    }
