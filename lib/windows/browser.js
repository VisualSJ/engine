...
let save = function() {
    ...
    // 判断是否需要记录窗口
    // 如果窗口内的 layout 信息内记录的 panel 都不需要记录，则删除这个窗口的信息
    for (let i = 0; i < array.length; i++) {
        if (!array[i].userData.layout) {
            continue;
        }
        const layout = array[i].userData.layout.layout;

        function check(layout) {
            if (layout.panels) {
                const save = layout.panels.some((name) => {
                    const info = panel.queryInfo(name);
                    if (!info) {
                        return true;
                    }
                    return !!info.userData.save;
                });
                return save;
            }
