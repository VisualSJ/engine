...
(() => {
    ...
    const json = fse.readJsonSync(configFile);
    project.init(json);
})();
...
class ProjectManager {
    ...
    registerTemplate(path) {
        ...
        this._templates[pkgJson.type].push({name: pkgJson.name, path});
    }
    ...
    add(type, path) {
        if (['2d', '3d'].indexOf(type) === -1) {
            throw new Error('Project type is not exists.');
        }
        // 新增项目
        project.add(path);
    }
...
// 打开项目
ipc.on('editor3d-lib-project:open-project', (event, path) => {
    // electron 程序地址
