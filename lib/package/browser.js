...
class PackageManager extends EventEmitter {
    ...
    async load(path, type = 'internal') {
        ...
        if (Object.keys(name2package).indexOf(name) !== -1 && lockPackageNames.indexOf(name) !== -1) {
 ...
 * @param {*} path
 */
function watchRequire(pkgName, path) {
    ...
    pkgCaches[pkgName] = [];
    pkgCaches[pkgName].push(mainPath);
 ...
 * @param {*} pkgName
 */
function clearPkgCaches(pkgName) {
    let cacheModule = pkgCaches[pkgName];
    for (let item of cacheModule) {
