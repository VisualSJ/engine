...
class PackageManager extends EventEmitter {
    ...
    scan(dir) {
        ...
        const globalDisable = this.profiles.global.get('disable') || [];
        const localDisable = this.profiles.local.get('disable') || [];
        const localEnable = this.profiles.local.get('enable') || [];
     ...
     */
    register(path) {
        this.path2info[path] = new PackageInfo(path);
